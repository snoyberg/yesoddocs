...
parseChapter = force "Missing chapter" $ tagName "chapter" ((,) <$> requireAttr "id" <*> requireAttr "status") $ \(slug, status') -> do
    ...
    msynopsis <- tagName "synopsis" (requireAttr "name") return
...
parseExample href = do
    contents <- liftIO $ TIO.readFile $ "../yesod-examples/src/" ++ T.unpack href ++ ".lhs"
...
parseSnippet href = do
    contents <- liftIO $ TIO.readFile $ "../snippets/" ++ T.unpack href ++ ".hs"
